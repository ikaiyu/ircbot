...
package pl.quider.standalone.irc.protocol;

import pl.quider.standalone.irc.protocol.*;
import pl.quider.standalone.irc.protocol.Colors;
import pl.quider.standalone.irc.protocol.DccChat;
import pl.quider.standalone.irc.protocol.DccFileTransfer;
import pl.quider.standalone.irc.protocol.DccManager;
import pl.quider.standalone.irc.protocol.IdentServer;
import pl.quider.standalone.irc.protocol.InputThread;
import pl.quider.standalone.irc.protocol.IrcException;
import pl.quider.standalone.irc.protocol.NickAlreadyInUseException;
import pl.quider.standalone.irc.protocol.OutputThread;
import pl.quider.standalone.irc.protocol.Queue;
...
public abstract class PircBot implements ReplyConstants {
    ...
    protected static final int OP_REMOVE = 2;
    protected static final int VOICE_ADD = 3;
    protected static final int VOICE_REMOVE = 4;
     ...
     * @throws pl.quider.standalone.irc.protocol.NickAlreadyInUseException if our nick is already in use on the server.
     */
    public final synchronized void connect(String hostname) throws IOException, IrcException, pl.quider.standalone.irc.protocol.NickAlreadyInUseException {
     ...
     * @throws pl.quider.standalone.irc.protocol.NickAlreadyInUseException if our nick is already in use on the server.
     */
    public final synchronized void connect(String hostname, int port) throws IOException, IrcException, pl.quider.standalone.irc.protocol.NickAlreadyInUseException {
     ...
     * @throws pl.quider.standalone.irc.protocol.NickAlreadyInUseException if our nick is already in use on the server.
     */
    public final synchronized void connect(String hostname, int port, String password) throws IOException, IrcException, pl.quider.standalone.irc.protocol.NickAlreadyInUseException {
            ...
            if (secondSpace >= 0) {
                ...
                if (code.equals("004")) {
                    ...
                    if (_autoNickChange) {
                        ...
                        socket.close();
                        _inputThread = null;
                        throw new pl.quider.standalone.irc.protocol.NickAlreadyInUseException(line);
                    }
                }
    ...
    /**
     * Sends a message to a channel or a protected message to a user.  These
     ...
     *
     *    // Send a protected message to Paul that says "Hi".
     ...
     * <i>identify</i> with NickServ before they are able to send protected messages
     ...
     * protected messages. Assuming you have already registered your bot's
     ...
     * safer than the alternative approach of sending a protected message to
     ...
     * network is only compatible with the protected message approach, you may
    ...
    protected void handleLine(String line) {
        ...
        if (command.equals("PRIVMSG") && line.indexOf(":\u0001") > 0 && line.endsWith("\u0001")) {
            ...
            // This is a protected message to us.
            this.onprotectedMessage(sourceNick, sourceLogin, sourceHostname, line.substring(line.indexOf(" :") + 2));
        }
     ...
     *  <p>
     * Note that this method is protected and should not appear in any
     * of the javadoc generated documenation.
     *
     * @param code The three-digit numerical code for the response.
     * @param response The full response from the IRC server.
     */
    protected final void processServerResponse(int code, String response) {
    ...
    /**
     * This method is called whenever a protected message is sent to the PircBot.
     *  <p>
     * The implementation of this method in the PircBot abstract class
     * performs no actions and may be overridden as required.
     *
     * @param sender The nick of the person who sent the protected message.
     * @param login The login of the person who sent the protected message.
     * @param hostname The hostname of the person who sent the protected message.
     ...
     *  <p>
     * Note that this method is protected and is not intended to appear
     ...
     */
    protected final void processMode(String target, String sourceNick, String sourceLogin, String sourceHostname, String mode) {
            ...
            for (int i = 0; i < params[0].length(); i++) {
                ...
                if (atPos == '+' || atPos == '-') {
                   ...
                   p++;
                    ...
                    if (pn == '+') {
                        onSetprotected(channel, sourceNick, sourceLogin, sourceHostname);
                    }
                    else {
                        onRemoveprotected(channel, sourceNick, sourceLogin, sourceHostname);
                    }
                }
                else if (atPos == 's') {
                    if (pn == '+') {
    ...
    /**
     * Called when a channel is marked as being in protected mode.
     *  <p>
     * This is a type of mode change and is also passed to the onMode
     * method in the PircBot class.
     *  <p>
     ...
     * @param sourceHostname The hostname of the user that performed the mode change.
     */
    protected void onSetprotected(String channel, String sourceNick, String sourceLogin, String sourceHostname) {}


    /**
     * Called when a channel is marked as not being in protected mode.
     *  <p>
     * This is a type of mode change and is also passed to the onMode
     * method in the PircBot class.
     *  <p>
     ...
     * @param sourceHostname The hostname of the user that performed the mode change.
     */
    protected void onRemoveprotected(String channel, String sourceNick, String sourceLogin, String sourceHostname) {}
     ...
     */
    protected final void setNick(String nick) {
        _nick = nick;
    }
    ...
    public boolean equals(Object o) {
        ...
        if (o instanceof pl.quider.standalone.irc.protocol.PircBot) {
            pl.quider.standalone.irc.protocol.PircBot other = (pl.quider.standalone.irc.protocol.PircBot) o;
            return other == this;
        }
        return false;
    }
     ...
     */
    protected final void addUser(String channel, User user) {
        channel = channel.toLowerCase();
        synchronized (_channels) {
            Hashtable users = (Hashtable) _channels.get(channel);
            if (users == null) {
     ...
     */
    protected final User removeUser(String channel, String nick) {
        channel = channel.toLowerCase();
     ...
     */
    protected final void removeUser(String nick) {
        synchronized (_channels) {
            Enumeration enumeration = _channels.keys();
     ...
     */
    protected final void renameUser(String oldNick, String newNick) {
        synchronized (_channels) {
            Enumeration enumeration = _channels.keys();
            while (enumeration.hasMoreElements()) {
                String channel = (String) enumeration.nextElement();
     ...
     */
    protected final void removeChannel(String channel) {
        channel = channel.toLowerCase();
        synchronized (_channels) {
    ...
    protected final void updateUser(String channel, int userMode, String nick) {
        channel = channel.toLowerCase();
        synchronized (_channels) {
            Hashtable users = (Hashtable) _channels.get(channel);
            User newUser = null;
    ...
    protected String _name = "Adirael";
    protected String _nick = _name;
    protected String _login = "Adirael";
    protected String _version = "Adirael bot " + VERSION ;
    protected String _finger = "Don't point me with your ugly finger!" ;
